...
namespace MonsterCardGame
{
    internal class Program
    {
        ...
        {
            TcpListener listener = new(new IPEndPoint(IPAddress.Loopback, 8080));
            Socket serverSock = new(
                AddressFamily.InterNetwork,
                SocketType.Stream,
                ProtocolType.Tcp
            );
            serverSock.Bind(new IPEndPoint(IPAddress.Loopback, 8080));
            ...
            while (true)
            {
                ...
                Thread thread = new(() => HandleClientRequest(clientSock));
                thread.Start();
            }
        }

        private static void HandleClientRequest(Socket clientSock)
        {
            using (NetworkStream networkStream = new(clientSock))
        ...
        private static void MapRequestFragments((string request, List<string> headers, string content) fragments, out HttpRequestObj httpRequestObj)
        {
            ...
            httpRequestObj = new()
            {
                RequestType = requestFragments[0],
